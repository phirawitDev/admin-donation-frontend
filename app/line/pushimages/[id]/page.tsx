...
export default function PushImagePage() {
  ...
  const promptPin = useCallback(() => {
    Swal.fire({
      title: "กรุณาใส่รหัส PIN เพื่อเข้าถึง",
      input: "password",
      ...
      if (result.isConfirmed) {
        ...
        const expiryTime = Date.now() + twoHours;
        localStorage.setItem("pinAuthExpiry", expiryTime.toString());
  ...
  useEffect(() => {
    ...
    if (expiryTime && Date.now() < parseInt(expiryTime, 10)) {
      ...
      localStorage.removeItem("pinAuthExpiry");
      if (authenticated) {
