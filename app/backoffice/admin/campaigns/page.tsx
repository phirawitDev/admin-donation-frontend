...
export default function BlankPage() {
  ...
  const fetchCampaigns = async () => {
    const url = process.env.NEXT_PUBLIC_API_URL + "/campaigns/list";
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      if (result.isConfirmed) {
        const url = process.env.NEXT_PUBLIC_API_URL + `/campaigns/delete/${id}`;
        const headers = getAuthHeaders();
    ...
    <div className="flex flex-col w-full gap-2">
      <div className="flex flex-col xl:flex-row gap-2 xl:gap-0 justify-between items-center p-4">
        ...
        <div className="w-full xl:w-auto flex flex-col xl:flex-row gap-2">
          <Link
            href="/backoffice/admin/campaigns/close"
            className="btn btn-error w-full xl:w-40"
          >
            <IoIosCloseCircle /> กองบุญที่ปิดแล้ว
          </Link>
          <button
        ...
        <dialog id="modalUpdate" className="modal">
          <div className="modal-box">
            ...
            <div className="mt-6 w-full flex flex-col gap-2">
              ...
              <div className="flex flex-col xl:flex-row gap-1 xl:items-center">
                <p className="w-40">จำนวนเปิดรับ:</p>
                <input
                  type="number"
                  className="input w-full"
                  placeholder="กรอกจำนวนที่เปิดร่วมบุญ"
                  onChange={(e) =>
